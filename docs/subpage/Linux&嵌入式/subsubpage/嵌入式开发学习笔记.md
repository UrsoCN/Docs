...
> 这不是一套学校式的嵌入式课程，而是一个已有编程基础的开发者，围绕真实固件项目重新梳理并补全技术栈的过程记录。学习采用“项目驱动、纵向打通、按需深入、实践验收”的方式：不要求先把所有基础学完再碰项目，也不以读完文档作为完成标准。
>
> 第 1～3 课是在初始大纲之上，随着实际问题和资料查阅逐步补全形成的。它们会自然包含后续主题的背景知识，可作为专题资料反复查阅；后续课程不再追求把相关知识一次讲全，而以解决一个具体工程问题、完成一个可验证闭环为主。

### 阶段 1：建立全局认识与工程闭环（第 1-4 课）

| # | 主题 | 说明 | 状态 |
|---|------|------|:----:|
| 1 | [**嵌入式系统是什么 + 本项目全局地图**](./第_1_课：嵌入式系统是什么+本项目全局地图.md) | 概念、裸板/RTOS/Linux 三层对比、开发流程、本项目全局地图 | ✅ |
| 2 | [**ESP32-S3 芯片与外设**](./第_2_课：ESP32-S3芯片与外设.md) | CPU 架构、GPIO、I2C、I2S、SPI、UART、USB、Timer/WDT、存储 | ✅ |
| 3 | [**FreeRTOS 实时操作系统**](./第_3_课：FreeRTOS实时操作系统.md) | 任务、调度、优先级、队列、信号量、看门狗 | ✅ |
| 4 | **ESP-IDF 工程闭环与基础调试** | 项目结构、CMake、sdkconfig、构建、烧录、日志、map/size、panic/backtrace；完成一次“修改→构建→烧录→观察→定位”闭环 | ⬜ |

### 阶段 2：打通硬件控制与音频数据链（第 5-8 课）

| # | 主题 | 工程主线与预期产物 | 状态 |
|---|------|------|:----:|
| 5 | **I2C 控制链与音频 Codec** | 从 ESP32-S3 读写 XVF3800/AIC3104 寄存器；用日志或逻辑分析仪验证一次真实事务 | ⬜ |
| 6 | **I2S、DMA 与音频格式** | 理解主/从、时钟、采样率、位深、声道和 DMA；捕获或构造一段可验证的 PCM 数据 | ⬜ |
| 7 | **FreeRTOS 工程实践** | 观察真实任务的核心、优先级、栈高水位和队列积压，并完成一次故障注入 | ⬜ |
| 8 | **实时音频处理管道** | 打通 XVF3800/Codec → I2S → DMA → 缓冲/队列 → DSP 的最小链路，测量帧大小、缓冲时长和处理预算 | ⬜ |

### 阶段 3：打通 USB、控制面与语音能力（第 9-11 课）

| # | 主题 | 工程主线与预期产物 | 状态 |
|---|------|------|:----:|
| 9 | **USB UAC 音频链路** | 理解枚举、描述符、端点和同步传输；让 PC 稳定识别设备并验证一条音频收发路径 | ⬜ |
| 10 | **CDC 与 USB 复合设备** | 理解 CDC、TinyUSB 和端点规划；验证 JSON 命令的收发、异常输入及兼容性 | ⬜ |
| 11 | **语音处理与识别** | 在已打通的音频链上依次加入重采样、NS、AGC、VAD、MultiNet/KWS，并分别验证各处理阶段 | ⬜ |

### 阶段 4：存储、升级与系统可靠性（第 12-15 课）

| # | 主题 | 工程主线与预期产物 | 状态 |
|---|------|------|:----:|
| 12 | **Flash、分区与持久化** | 从 Flash 特性理解分区表、NVS/SPIFFS 和模型存储；核对实际容量、镜像布局及擦写边界 | ⬜ |
| 13 | **OTA 升级与安全回滚** | 实现 CDC-OTA、双槽切换、完整性校验和失败回滚，并模拟传输中断或掉电 | ⬜ |
| 14 | **全双工与性能分析** | 分析 DMA、任务调度和时钟域；定位 underrun、overrun、丢帧及性能瓶颈 | ⬜ |
| 15 | **测试、故障恢复与长期稳定性** | 建立断言、看门狗、core dump、故障注入和长稳测试，覆盖典型异常 | ⬜ |

### 阶段 5：产品化补全（第 16-17 课）

| # | 主题 | 工程主线与预期产物 | 状态 |
|---|------|------|:----:|
| 16 | **电源管理与资源优化** | 休眠模式、PSRAM/片内存分配、功放控制，以及功耗、内存和延迟的权衡 | ⬜ |
| 17 | **量产与固件发布** | 工厂烧录、版本管理、Secure Boot/Flash Encryption、密钥、校准、发布和回退流程 | ⬜ |
...
每一阶段先打通一条最小可运行链路，再深入其内部原理。遇到 C 语言、链接、硬件电气或操作系统知识缺口时，围绕当前问题补充，不为了形式完整而脱离项目铺开全部学科。

需要贯穿补全的通用能力包括：

- 嵌入式 C：`volatile`、内存布局、对齐、生命周期、位操作、未定义行为；
- 构建与启动：编译、链接、section、map 文件、Bootloader 和启动流程；
- 硬件基本功：电平、上拉、驱动能力、电源、复位、时钟和信号完整性；
- 调试与测量：日志、backtrace、JTAG、万用表、示波器和逻辑分析仪；
- 并发与可靠性：竞态、临界区、优先级反转、超时、掉电恢复和长期运行；
- 测试：主机侧单元测试、板上集成测试、故障注入和回归记录。

### 2. 每课形成实践闭环

后续课程原则上包含以下五项，不能只以“读完/听懂”作为完成：

1. **源码阅读**：定位概念在项目中的入口、调用路径和配置来源；
2. **最小实验**：做一个范围可控的修改，并观察符合预期的结果；
3. **故障注入**：主动制造超时、断连、缓冲满或配置错误等失败；
4. **观测证据**：保存日志、波形、任务统计、内存数据或抓包结果；
5. **结论沉淀**：记录现象、原因、验证方法、修复方式和遗留问题。

### 3. 分层记录完成状态

课程表中的 ✅ 只表示课程文档已经完成。具体能力额外按以下层级记录：

| 层级 | 含义 |
|------|------|
| **已阅读** | 能复述核心概念，知道相关资料在哪里 |
| **已复现** | 按已有步骤在真实环境中得到预期结果 |
| **已修改** | 能独立改变配置或代码，并预测结果 |
| **已排障** | 能根据日志、波形或统计数据定位真实/注入故障 |
| **已沉淀** | 将可复用的结论、工具或检查清单写回知识库 |

### 4. 区分平台事实与通用原理

学习 RTOS、中断和启动机制时，要区分“通用原理”“ARM Cortex-M 示例”和“ESP32-S3/Xtensa 的实际实现”。涉及芯片容量、栈单位、时钟源、中断控制器、分区布局等可验证事实时，以当前项目配置、ESP-IDF 对应版本和芯片官方文档为准，并在后续相关课程中核对；已有课程保留为学习过程记录，不在本次路线调整中改写。
