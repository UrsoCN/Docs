...
本项目选择 **16kHz 采样率**，Nyquist 频率 = 8kHz，能覆盖宽带语音识别所需的大部分信息。齿音在 8kHz 以上仍可能有能量，因此“全部有效信息”并不严格；这里是在识别收益、模型训练制式与计算量之间取舍。
...
> **定理**：若模拟信号严格带限于 $|f| < B$，且采样率 $f_s > 2B$，理论上可以用理想 sinc 插值从无限长、无噪声的采样序列中重建原信号。工程上不会把信号带宽顶到 $f_s/2$：模拟抗混叠滤波器需要非零的过渡带。
...
| 48kHz（专业） | 过渡带宽裕，保留超宽带细节 | 相对 16kHz 是 3 倍数据量；若不先降采样，前端计算量也明显增加 | ❌ 与现有 16kHz 模型不匹配 |

> **项目证据**：`afe_config.yaml` 中的采集链路与 ASR Zipformer encoder 都按 16kHz Mono 设计。整个链路**不通过重采样**——这既避免额外计算，也避免重采样器引入的滤波延迟和频带差异（课程 18 详述）。设备名本身不能证明采样率，仍应以 ALSA hw params、配置字段或实际采集日志为准。
...
其中输入范围写作 $[-A,A)$。S16 的整数码值为 $[-32768,32767]$，共有 $2^{16}$ 个量化级；“符号位占掉一位”只是在解释数值范围，不表示它少了一个比特的量化精度。
...
| 32 bit float | 约 24 bit 有效尾数精度；动态范围与定点 SQNR 不能直接类比 | 仅在中间处理时使用（Float32MultiArray） |
...
| 参数 | 含义 | 典型值 | 对结果的影响 |
|------|------|--------|-------------|
| **N (FFT 点数)** | 每帧做 N 点 FFT，可大于窗长（零填充） | 512 / 1024 | 频点间隔为 $f_s/N$；单纯零填充只让频谱采样更密，不提高分辨两个近频率成分的真实能力 |
| **窗长** | 每帧包含的采样点数 | 通常 = N（可补零） | 越长 → 频率分辨率越高 |
| **跳步 H** | 相邻帧的起始偏移 | N/4 ~ N/2 | 越小 → 时间分辨率越高，计算量越大 |

> **注意**：窗长决定一帧实际观察了多久，FFT 点数决定返回多少频率采样点。二者相等很常见，但不是同一参数。本项目具体模型参数以后文源码核验表为准：GTCRN 使用 512 点窗和 512 点 FFT，而不是 1024 点窗。
...
> **窗函数选择的实战原则**：
> 1. 语音信号默认用 **Hamming 窗**
> 2. 若要分开两个相近频率，关键是更长的有效窗长（更窄主瓣）；若强分量的旁瓣遮住弱分量，则需要更低旁瓣的窗。不能只用“Hann 比 Hamming 更好”概括这两个不同问题
...
# 3. STFT → 频谱图
n_fft = 1024
hop = 256  # 75% overlap（重叠率 = 1 - hop / n_fft）
...
**潜在收益**：省掉 iSTFT、一次 PCM 搬运和 ASR STFT；实际节省量必须用目标设备上的 profile 证明，不能从算子数量直接推导百分比。

但这里有一个关键限制：GTCRN 的 32ms/sqrt-Hann/16ms 网格不等价于 ASR 的 25ms/Povey/10ms 网格。把 GTCRN 的 257-bin 频谱直接乘 Mel 滤波器，得到的不是 ASR 训练时见过的 FBank 分布。可行实现通常需要以下二选一：

1. 增强后仍恢复为时域波形，再按 ASR 参数重新分帧；这样只能省接口搬运，不能省 ASR STFT。
2. 重新训练或适配 ASR，使它接受 GTCRN 的帧率、窗函数和增强频谱；这已经是模型级联合设计，不是单纯增加 `AcceptFeatures()` 接口。
...
| **频谱图 (Spectrogram)** | STFT 随时间排列的二维表示；颜色可画幅度 $|X|$ 或功率 $|X|^2$，转 dB 时分别用 $20\log_{10}$ 或 $10\log_{10}$ |
...
### Q1：把 48kHz 音频每三个点取一个，就能得到 16kHz 吗？

不能直接抽点。原信号中 8kHz 以上的成分会混叠到新频带，必须先低通滤波，再按 3 倍抽取。工程重采样器的主要成本和延迟也来自这一步滤波。

### Q2：16-bit PCM 转成 float32 后，音质会自动提升吗？

不会。常见转换只是 `float_sample = int16_sample / 32768.0`，原有量化误差仍在。float32 的价值是给后续乘加、滤波和归一化留下余量，避免处理中间结果反复截断或溢出。

### Q3：峰值归一化后，两个录音的响度就可比了吗？

不一定。峰值只描述最大瞬时幅度；感知响度还受时长、频谱和动态范围影响。排查 ASR 输入可同时看 peak、RMS/LUFS、削波比例和直流偏置。尤其要警惕整数 PCM 未除以 32768 就送入模型，或归一化两次导致幅度过小。
