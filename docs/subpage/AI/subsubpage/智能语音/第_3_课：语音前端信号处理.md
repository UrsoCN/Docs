...
拖尾（hangover）解决了“已进入语音后，短暂停顿导致过早结束”的问题。它约束的是 **speech→silence 的释放时间**，不是最小语音段长度；silence→speech 通常另有 onset/debounce 规则，两者应分开调。
...
**特征**：输入帧只能是 10/20/30ms，内部按采样率下采样并计算 6 个子带的能量特征；总能量还用于低能量快速判决。具体边界由实现中的滤波器组决定，不应把“80/10ms”理解成 80 个子帧。
...
- $\mu$ 大 → 收敛快，但稳态误差大（抖动）
- $\mu$ 小 → 收敛慢，但稳态误差小（平滑）
- SpeexDSP 实际使用频域 **MDF（multidelay block frequency-domain adaptive filter）** 及双滤波器等稳定化策略。LMS/NLMS 适合建立自适应滤波直觉，但不能据此推断 SpeexDSP 的逐样本更新行为

**滤波器长度 $L$ 的含义**：

$$L = 4096 \text{ samples} = \frac{4096}{16000} = 256\text{ms}$$

这意味着自适应滤波器能表示约 256ms 的**回声路径冲激响应尾长**，不等于允许 far/mic 两路存在 256ms 的未知整体错位。声卡、buffer 和路由造成的 bulk delay 应先做延迟估计与对齐；对齐之后，filter length 才用于覆盖直达声和反射尾部。RT60 也不等于必须完整覆盖的滤波器长度，最终应以 ERLE、双讲表现和 CPU profile 选型。
...
- 不同 AEC 实现会采用相关性、相干性、双滤波器发散检测等策略。SpeexDSP 的 MDF 实现包含前景/背景滤波器与泄漏估计，不能仅概括为一个独立的 Geigel 门限
...
**3. Mask-based 方法**：模型可预测幅度掩码、复数比率掩码，或直接估计增强后的实部/虚部。只有使用 sigmoid 的幅度 mask 才天然限制在 0 到 1；复数 mask 可以修正相位，某些 ratio mask 的幅值也可大于 1：

$$\hat{S}[t,k] = M[t,k] \cdot Y[t,k], \quad M \text{ 可为实数或复数}$$
...
> **工程验证**：本项目 AFE 处理器链的注册顺序就是 `AEC → GTCRN → VAD`（`afe_io.cpp` 中的 `AddProcessor` 调用）。这是合理默认值，但不是脱离实现的普适定律。改变顺序必须重新验证 far-only、near-only、double-talk 和噪声突变四类场景。
...
不够。占比没有时间标注就无法区分漏检和虚警。至少报告 frame-level precision/recall、起始触发延迟、结束拖尾延迟，并按安静、稳态噪声、瞬态噪声和回声场景分桶。语音助手还应测每小时误触发和首音节截断率。

### Q2：怎样评估 AEC？

far-only 看 ERLE 和残余回声；near-only 检查近端失真；double-talk 检查滤波器是否发散、近端语音是否被抑制。还要记录参考路与麦克风路的 delay drift、削波和时钟偏差。只在单讲样本上得到很高 ERLE，不能证明 barge-in 可用。

### Q3：降噪模型的 SNR 提升了，为什么 ASR 反而更差？

增强模型可能提高波形指标，却抹掉清辅音或制造训练集中没有的伪影。应联合观察 SI-SDR/STOI/PESQ（各自都有局限）、ASR WER/CER、VAD 端点和主观听测；最终以真实下游任务和目标噪声分布为准。
